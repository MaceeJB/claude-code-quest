...
window.CURRICULUM = [
  {
    ...
    lesson: [
      { heading: "Three ways to install", body: "Native install (curl/irm scripts that auto-update), Homebrew (manual updates), or a package manager (apt, dnf, apk, winget). On Windows, installing Git for Windows lets Claude Code use Bash; without it, it falls back to PowerShell." },
      { heading: "Starting a session", body: "The <code>claude</code> command opens an interactive session. On launch it loads your CLAUDE.md files and auto memory, then shows a welcome screen. You log in on first use with a Claude subscription, Console account, or a cloud provider (Bedrock, Vertex AI, Foundry); credentials persist across sessions." },
      { heading: "One-off, no session", body: "<code>claude -p \"your prompt\"</code> runs a single task without opening a session — perfect for scripts, pre-commit hooks, and CI/CD." }
    ],
    quiz: [
