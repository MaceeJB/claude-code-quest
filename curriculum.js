...
window.CURRICULUM = [
  {
    ...
    lesson: [
      ...
      { heading: "Starting your first session", body: "In the desktop app, open the <b>Code</b> tab, click <b>New session</b>, then <b>Select folder</b> to point Claude at the folder you want to work in, no commands to type. (The Select folder button only shows when starting a new session; if you're inside an old session, click New session first.) <i>Terminal:</i> type <code>claude</code> inside a project folder instead. The first time, you log in (a Claude subscription, Console account, or a cloud provider such as Bedrock or Vertex AI), and your login is remembered after that. As it starts, Claude automatically loads any <code>CLAUDE.md</code> files and its own <b>auto memory</b>, saved notes about your project. So it begins already knowing your context instead of from a blank slate." },
    ...
    quiz: [
      { q: "Most of the team will use the desktop app. How do you open a project there?", options: ["Type claude in a terminal", "Open the Code tab, click New session, then Select folder", "Run an install script first", "Email IT for access"], answer: 1, explanation: "In the desktop app: Code tab, New session, then Select folder to pick your project folder. No terminal needed." },
    ...
    challenge: [
      { text: "Open the Claude desktop app, go to the <b>Code</b> tab, and click <b>New session</b> (left sidebar). Then click <b>Select folder</b> and pick the folder you want to work in. (Terminal users: run <code>claude</code> inside the folder instead.)", link: DOC + "desktop-quickstart" },
      ...
      { text: "<b>Week 1 Sprint kickoff, Learn something new:</b> pick a subject you're curious about (a hobby, a tool, a topic at work). Make an empty folder and open it in Claude Code (desktop: <b>New session</b>, then <b>Select folder</b>; terminal: run <code>claude</code> inside it), and <b>you choose what to build</b> over the next 5 days: a <b>study guide</b>, a set of <b>flashcards</b>, a <b>self-quiz</b>, a <b>glossary</b>, or a <b>learning roadmap</b>. To start, ask Claude to scaffold a <code>learning-plan.md</code> that breaks your subject into about 5 subtopics.", link: DOC + "quickstart", capstone: true }
    ]
  },
  {
    day: 2,
  ...
  },
  {
    ...
    quiz: [
      ...
      { q: "How do you switch permission modes?", options: ["The mode selector by the send button (or Shift+Tab in the terminal)", "Press Tab", "Run /quit", "You can't change them"], answer: 0, explanation: "Desktop has a mode selector next to send; the terminal cycles default → auto-accept edits → plan mode with Shift+Tab." },
  ...
  },
  {
    ...
    lesson: [
      ...
      { heading: "/model and /effort: dial cost vs. depth", body: "<code>/model</code> switches which model you're using (such as fable, opus, sonnet, or haiku), and <code>/effort</code> trades how hard Claude thinks against speed and cost. Use a lighter touch for simple chores, more depth for thorny problems." },
      { heading: "Meet Fable 5, the newest model", body: "Anthropic released <b>Fable 5</b> in June 2026, and it's the most capable model they've ever made generally available. What makes it different for an operator: <b>the longer and more complex the task, the bigger its lead</b>. It plans its approach, checks its own progress against the goal, and refines as it goes, so it shines on big multi-step jobs like a full audit or a messy multi-file cleanup. It's rolling out across Claude plans now; run <code>/model</code> to see if it's available to you. For quick chores, a lighter model is still the smart, cheaper pick." },
    ...
    quiz: [
      ...
      { q: "Fable 5, the newest model, is the strongest choice for which kind of work?", options: ["Quick one-line chores", "Long, complex multi-step jobs like a full audit", "Only writing code", "It's always the wrong choice"], answer: 1, explanation: "Fable 5's lead grows with task length and complexity; it plans, checks its progress, and refines as it goes. Lighter models stay the smart pick for quick chores." }
    ],
    challenge: [
      ...
      { text: "Try <code>/model</code> to see which models you can switch to, and check whether <b>Fable 5</b> is available on your plan yet.", link: DOC + "slash-commands" },
...
window.DEEP_DIVE = {
  ...
  4: [
    ...
    { q: "When should you reach for /model or /effort?", a: "Use a lighter model or lower effort for simple chores (faster, cheaper), and more depth for thorny problems. The new <b>Fable 5</b> is the top pick for long, complex jobs. Match the horsepower to the task instead of always running at full power." },
