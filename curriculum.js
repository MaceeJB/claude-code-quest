...
window.CURRICULUM = [
  {
    ...
    quiz: [
      { q: "Most of the team will use the desktop app. How do you open a project there?", options: ["Type claude in a terminal", "Open the Code tab and click Select folder", "Run an install script first", "Email IT for access"], answer: 1, explanation: "In the desktop app you open the Code tab and pick your project folder — no terminal needed." },
  ...
  },
  {
    ...
    lesson: [
      ...
      { heading: "Adding one (or just asking IT)", body: "In the desktop app you add connections through the <b>connector settings</b> — a UI, no commands to type. <i>Terminal:</i> <code>claude mcp add --transport http &lt;name&gt; &lt;url&gt;</code>, with <code>--scope project</code> to write it to <code>.mcp.json</code> so your whole team shares it; <code>claude mcp list</code> shows what's configured. In practice, your team lead or IT often sets these up once and shares the <code>.mcp.json</code>, so everyone is connected without doing the setup themselves." },
